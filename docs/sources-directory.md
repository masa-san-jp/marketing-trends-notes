...
書く。**未確認のものは未確認と明記する**（このKBの根本規律）。`python3 tools/linkcheck.py` を
制限のない環境で回し、確認できたものからこの表を昇格させる。

## 取得手段の現実（2026-08-10 実測）

**この環境では、探せるが読めない。** 実測した結果:

| 手段 | 状態 |
|---|---|
| **WebSearch** | **通る。** 検索結果のタイトル・URL・要約が取れる |
| WebFetch | **egress allowlist 依存。** `developer.apple.com` は通るが、`soumu.go.jp`・`current.ndl.go.jp` は `EGRESS_BLOCKED` |
| curl | 同上（CONNECT に 403） |

つまり **「何があるか」は分かるが、「原典に何と書いてあるか」は多くの場合読めない**。だから
evidence の `retrieved` を必須にしている——検索要約で得た数字に官公庁のURLを添えると、
読んでいない資料を読んだことにできてしまうから（`retrieved: summary` と明記すれば、それは
「概要は掴んだが原典未読」という正しい記録になる）。

**この制約を外すには**、Claude Code on the web の environment 設定でネットワーク許可ドメインに
統計系サイトを追加する（<https://code.claude.com/docs/en/claude-code-on-the-web>）。
許可されれば `retrieved: primary` に昇格させる作業（数字の突き合わせ）が回せるようになる。

**いまの `primary_read_ratio` の数字を過信しない。** 2026-08-10 時点は 0.50 で閾値 0.30 を満たして
いるが、これは母数が2件で、そのうち1件がたまたま許可ドメイン（`developer.apple.com`）の資料
だったからにすぎない。**到達できない出典から trend を積むほどこの比率は下がる**ので、件数が
増えるにつれて環境制約がそのまま指標に効いてくる。空欄を空欄として見せるのがこのKBの流儀なので、
下がったら下がったまま表に出す。
...
| 情報源 | 何が取れるか | URL | 確認状況 |
|---|---|---|---|
| 総務省 情報通信メディアの利用時間と情報行動に関する調査 | メディア別・年代別の利用時間・利用率（年次）。**このKBで最も使う統計** | https://www.soumu.go.jp/iicp/research/results/media_usage-time.html | **原典未読**（WebSearch で令和6年度の数値は取得済み・PDFは egress で読めず） |
| ↑ 令和6年度 報告書（2025年6月公表）報道資料 | 同上の該当年版 | https://www.soumu.go.jp/menu_news/s-news/01iicp01_02000125.html | **原典未読**（同上） |
| ↑ 令和6年度 報告書 本体PDF | 同上 | https://www.soumu.go.jp/main_content/001017160.pdf | **原典未読**（同上。許可されたら最初に読む1本） |
...
- 新しい情報源を使ったら、この表に**層と確認状況つきで**追記する（1調査1行の追記でよい）。
  確認状況は3段階——**確認済み**（原典を開いて読んだ）／**原典未読**（存在と概要は掴んだが
  本体を読んでいない）／**未確認**（到達すら試していない・失敗した）
