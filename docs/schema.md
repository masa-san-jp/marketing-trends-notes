...
### `evidence` — 主張ごとの根拠。2つの独立した軸で質を持つ

art-history の `claims` に当たる。ただしこのKBの目的は**外部の情報を収集・蓄積すること**なので、
根拠の質を2軸で持つ。**誰が出したか**（`certainty`）と、**自分がそれを読んだか**（`retrieved`）。
この2つは独立に動き、どちらが欠けても「出典付きの顔をした未検証」が体系に入る。

```yaml
evidence:
  - {field: stage, source: "https://...", certainty: independent, as_of: "2024", retrieved: summary}
```

#### `certainty` — 誰が出したか（利害があるか）

マーケの出典は誰かの売り物であることが多く、「研究の通説か仮説か」という美術史の軸より先に、
まず利害の軸で切る必要がある。
...
| `measured` | 自分で測った数字（自社GA・売上）。あれば最強だが、このKBは外部情報の収集が目的なので主役ではない |
...
#### `retrieved` — 自分がその出典に到達したか

| retrieved | 意味 |
|---|---|
| `primary` | **原典そのものを読んだ**（ページ・PDF・APIレスポンスを開いた） |
| `summary` | 検索結果の要約・二次記事の引用など、**経由して得た**（原典は未読） |

**なぜ必要か**: 検索で得た数字に官公庁のURLを添えると、読んでいない資料を読んだことにできてしまう。
形の上では出典があるので、読む側からは区別が付かない——art-history-notes が実測で見つけた
「実在するページから別言語版のURLを組み立てた偽の出典」と同じ型の事故になる。
美術史の「実物を見ていない作品は**実物未見**と明記する」に当たる規律を、機械可読にしたもの。

`summary` は悪ではない。**存在と概要を掴む段階では正しい状態**で、隠すのが悪い。
原典を読んだら `primary` に上げ、数字を突き合わせる（要約が丸めていたり、年度を取り違えている
ことがある）。被覆マップに「原典を実読していない」trend の一覧が出る。

#### `verified` の2つの関門

**`status: verified` を名乗るには、`independent` / `attested` / `measured` の根拠が最低1本、
かつ `retrieved: primary` の根拠が最低1本要る**（検証が両方を強制する）。
vendor と anecdotal をいくら重ねても通らないし、検索要約だけでも通らない。
verified に evidence が要る field は trend が `time` / `kind` / `stage`、practice が `time`。

各行は `as_of`（いつ時点の数字か）も必須——数字の鮮度は記述の鮮度と別に動く。
...
**深さは揃えない。** 自分の事業に効く trend を深く掘り、隣は名前だけ、が正しい状態。
...
- **practice**: `## 何をするか` → `## どのトレンドへの応答か` → `## 効いた条件・効かない条件` →
  `## 飽和度の判定` → `## 自分の事業にどう使うか`
...
- **試していない施策は「未実施」と明記する**（美術史の「実物未見」に当たる）
