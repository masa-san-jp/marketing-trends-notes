...
**根拠は2つの独立した軸で質を持つ。** `certainty` が**誰が出したか**（`independent` / `attested` /
`measured` / `vendor` / `anecdotal` / `hypothesis`）、`retrieved` が**自分がそれを読んだか**
（`primary` = 原典を開いた／`summary` = 検索要約など経由）。**verified を名乗るには
vendor 以外の根拠と、原典を実読した根拠の両方が必須**——検証が強制する。
検索要約に官公庁のURLを添えれば、読んでいない資料を読んだことにできてしまうから。
...
- **ベンダー発の数字は `vendor` と明記し、断定に使わない。** その主張で儲かる側の数字は
  「調査レポート」の顔で来る。
- **利害のない第三者の数字を最優先する**（官公庁統計・学術・業界団体）。
- **原典を読んでいない根拠は `retrieved: summary` と明記する。** 検索結果の要約から数字を取って
  出典URLだけ添えるのは、読んでいない資料を読んだことにする行為（美術史の「実物未見」に当たる）。
- **試していない施策は「未実施」と明記する。**
