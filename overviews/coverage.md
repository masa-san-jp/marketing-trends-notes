...
データの最新日: 2026-09-21 — `python3 tools/build_graph.py` が生成（手で書き換えない）
...
- stale_ratio: 0.00（上限 0.15・as_of=2026-09-21 時点）
- practice_linked: 60/61 の practice が responds_to を持つ（下限比率 0.5）
- resolved_predictions: 5/5 件の予測が答え合わせ済み
- forward_stated_ratio: 0.72（下限 0.0・第一節とpredictionsを持つtrend）
- intended_evidence_ratio: 0.65（下限 0.0・tense: intendedの根拠を持つtrend）
