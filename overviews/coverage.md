...
trend **15** 件（stub 0件は不算入）／内訳 {'draft': 3, 'stub': 0, 'verified': 12}
...
- per_category_min: 3/12 カテゴリが 2件以上
- stale_ratio: 0.00（上限 0.15・as_of=2026-08-11 時点）
- practice_linked: 13/14 の practice が responds_to を持つ（下限比率 0.5）
- resolved_predictions: 0/5 件の予測が答え合わせ済み

関係を持たない trend: trend/online-marketplace-transparency
...
**応答なきトレンド**
- オンラインモール取引透明化の制度化 に応答する practice が無い。打ち手に落ちていない（判断材料として未完成）
<!-- generated:audit:end -->
