---
id: trend/japanese-music-overseas-expansion
uri: urn:mtn:trend/japanese-music-overseas-expansion
type: trend
kind: demand-shift
stage: emerging
market: creator-economy
geo: global
label_ja: 日本音楽の海外収入・売上の可視化
label_en: Visibility of overseas revenue and sales for Japanese music
authority:
  wikidata: null
  none_reason: "「日本音楽の海外収入・売上の可視化」「Japanese music overseas revenue and sales」で検索したが、この産業全体の海外収入・売上を可視化する現象そのもののWikidata項目は確認できない（2026-08-11検索実施）"
time:
  start: "2024"
  end: ".."
  display: "日本音楽産業の海外収入・海外売上を産業横断で初めて推計し、継続把握の基盤を整える段階"
naming:
  self_identified: false
  named_by: null
  named_when: null
  original_label: 日本音楽産業の海外展開データ把握
  note: "「日本音楽の海外収入・売上の可視化」は、経済産業省の2024年調査結果を要約する記述的なラベル"
freshness:
  valid_as_of: "2026-08-11"
  recheck_by: "2026-09-11"
evidence:
  - {field: kind, source: "https://www.meti.go.jp/policy/mono_info_service/contents/2026/0430/musicindustry_data_report.pdf", certainty: independent, retrieved: primary, as_of: "2024"}
  - {field: stage, source: "https://www.meti.go.jp/policy/mono_info_service/contents/2026/0430/musicindustry_data_report.pdf", certainty: independent, retrieved: primary, as_of: "2024"}
  - {field: time, source: "https://www.meti.go.jp/policy/mono_info_service/contents/2026/0430/musicindustry_data_report.pdf", certainty: independent, retrieved: primary, as_of: "2024"}
predictions:
  - {claim: "2025年以降も日本音楽産業の海外収入・海外売上が同じ定義で再推計される", by: "2028-12", resolved: null, outcome: null}
channels: []
relations: []
sources:
  - https://www.meti.go.jp/policy/mono_info_service/contents/2026/260326.html
  - https://www.meti.go.jp/policy/mono_info_service/contents/2026/0430/musicindustry_data_report.pdf
status: verified
updated: 2026-08-11
---

# 日本音楽の海外収入・売上の可視化

## 何が変わったか

日本音楽産業の海外展開が、個別アーティストや作品の成功談ではなく、権利収入、音楽ソフト、配信、ライブ、その他を合算した産業統計として推計され始めた。経済産業省の調査では、2024年の海外収入は448.6億円、海外売上は1,239.5億円だった。訪日外国人による消費を含めると、それぞれ725.8億円、1,516.7億円となる。

この調査は初回の産業横断推計であり、過去年との同一定義の比較や、海外需要が何％伸びたかを示すものではない。したがって「市場が前年比で拡大した」とは断定せず、海外需要を測定・運用する基盤が立ち上がったトレンドとして扱う。

## kind と stage の判定

**kind: demand-shift とした。** 推計の対象は海外消費者・海外市場に由来する収入と売上であり、広告施策や政府支援額そのものではない。

**stage: emerging とした。** 2024年の産業規模は推計できたが、報告書自身が産業全体を継続的に把握する仕組みは未確立と整理している。同一定義の経年比較、分野別の市場浸透、個社の再現性はこれからである。

## 時間

初回の産業横断推計の対象年である `2024` とした。終点は `..`（継続的な把握を目指す段階）。

## チャネルと伝播

音楽配信、UGCプラットフォーム、海外公演、音楽ソフト、著作権・ライセンス、ファンクラブ、マーチャンダイズ、訪日外国人の国内消費など複数の取引経路がある。経路別の因果や新規顧客獲得は未確認である。

## 反証（これが偽なら何が観測されるか）

- 次回以降に同じ定義の産業推計が継続されず、海外収入・海外売上の把握が単発調査で終わる
- 配信・ライブ・権利収入の複数区分で海外取引が確認できず、初回推計が一部の事業者に偏っていたと分かる
- 国・地域別に見ると海外需要が一部作品・一部イベントに限られ、産業全体へ広がっていない

## 未着手

- 2025年以降の同一定義の推計と、2024年との比較可能性を確認する
- 国・地域、作品、配信、ライブ、権利、物販別に需要・収入・売上を分解する
- 推計対象企業、回答率、未観測取引、再現性を報告書の次版で確認する
