...
**効果未確認**:（対象事業でチェックアウトを運用していない）。決済手段の追加が購入率、客単価、再購入率に与える効果も測っていない。
...
**効果未確認**: 対象事業者に購入導線がある場合は、まず直近の決済失敗・返金・問い合わせを決済手段別に記録し、対応手段を増やす前後で
購入完了率と運用コストを分けて測る。外部資料では現時点でこのpracticeを試すチェックアウトがないため、効果の主張は置かない。
